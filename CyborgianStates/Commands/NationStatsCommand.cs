        ...
        private async Task ProcessResultAsync(Request request)
        {
            if (request.ExpectedReponseFormat == ResponseFormat.XmlResult && request.Response is XmlDocument nationStats)
            {
                ...
                _responseBuilder.Success()
                    ...
                    .WithField("Founded", $"{dateFounded.ToShortDateString()} ({founded})")
                ...
                _responseBuilder.WithField("Resident Since", $"{dateJoined.ToShortDateString()} ({(residencyYears < 1 ? "" : $"{residencyYears} y ")}{residencyDays} d)", string.IsNullOrWhiteSpace(officerPosition));
